...
if selected_stream != "All":
    filtered_df = filtered_df[filtered_df["Stream"] == selected_stream].reset_index(drop=True)
...
growth = ((total_actual - total_last_year) / total_last_year) * 100 if total_last_year > 0 else 0
...
with col1:
    ...
    fig1.add_trace(go.Scatter(
        x=filtered_df["Month"], y=filtered_df["Actual_Revenue"],
        mode="lines+markers", name="Actual",
        line=dict(color="#6bcb77", width=3)))
    fig1.add_trace(go.Scatter(
        x=filtered_df["Month"], y=filtered_df["Target_Revenue"],
        mode="lines+markers", name="Target",
        line=dict(color="#ffd93d", width=3, dash="dash")))
    fig1.add_trace(go.Scatter(
        x=filtered_df["Month"], y=filtered_df["Last_Year_Revenue"],
...
with col3:
    ...
    if len(df_growth) > 1:
        df_growth["Growth_%"] = df_growth["Actual_Revenue"].pct_change() * 100
        df_growth["Growth_%"] = df_growth["Growth_%"].fillna(0).round(1)
    else:
        df_growth["Growth_%"] = 0

    fig2 = px.bar(df_growth, x="Month", y="Growth_%",
                  color="Growth_%",
    ...
    st.markdown("""
    <div class='bubble'>📊 Green bars = growth months, Red bars = decline months!</div>
    """, unsafe_allow_html=True)
...
with col4:
    ...
    st.markdown(f"""
    <div class='bubble'>💼 <b>{top_stream}</b> is the biggest revenue stream!</div>
    """, unsafe_allow_html=True)
...
with col5:
    ...
    fig4.add_trace(go.Bar(
        x=filtered_df["Month"], y=filtered_df["New_Clients"],
        name="New Clients", marker_color="#4d96ff"))
    fig4.add_trace(go.Bar(
        x=filtered_df["Month"], y=filtered_df["Recurring_Clients"],
...
with col6:
    ...
    total_clients = total_recurring + total_new_clients
    retention = (total_recurring / total_clients * 100) if total_clients > 0 else 0
    st.markdown(f"""
    <div class='bubble'>🤝 <b>{total_new_clients}</b> new clients acquired!</div>
    <br>
